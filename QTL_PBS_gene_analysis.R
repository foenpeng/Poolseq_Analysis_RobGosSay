...

{
  ...
  PBS_gene<-fread("./Analyses_Poolseq/Results/Foen/PBS_gene.csv")
  PBS_slide_bp<-fread("./Analyses_Poolseq/Results/Foen/PBS_slide_20kb.csv")
...
##### Generate a table containing all the sliding windows fall to any QTL
{
  PBS_slide_bp[,c("window_join_start","window_join_end"):=list(window_start,window_end)]
  focal_qtl[,c("qtl_join_start","qtl_join_end"):=list(qtl.focal.region.start,qtl.focal.region.end)]
  focal_qtl_window<-focal_qtl[PBS_slide_bp,
                              nomatch = 0L,
                              on = c("qtl_join_start<=window_join_start","qtl_join_end>=window_join_end","LGn"),
                              allow.cartesian = TRUE]
}  

##### Generate a table containing all the genes fall to the significant PBS windows and inside QTL
{
  setkey(PBS_slide_bp,window_start,window_end)
  setkey(PBS_gene,start,stop)
  window_sig_gene<-foverlaps(PBS_slide_bp[PBS.r.focal | PBS.g.focal,],PBS_gene,by.x = c("window_start","window_end"), by.y=c("start","stop"))[LGn==i.LGn,]
  window_sig_gene<-unique(window_sig_gene,by="GeneID")
  focal_qtl[,c("qtl_join_start","qtl_join_end"):=list(qtl.focal.region.start,qtl.focal.region.end)]
  focal_qtl_gene<-focal_qtl[window_sig_gene,
                            nomatch = 0L,
                            on = c("qtl_join_start<=window_start","qtl_join_end>=window_end","LGn"),
                            allow.cartesian = TRUE]
}  

##### Save a copy of significant genes without duplicates
{
  focal_qtl_gene_significant<-focal_qtl_gene[,.SD,by=c("qtl.trait","LGn","GeneID")]
  focal_qtl_gene_significant[,paste0("In.", qtl_traits):=lapply(qtl_traits, function(i) as.integer(qtl.trait==i))]
  focal_qtl_gene_significant[,(37:42):=lapply(.SD, sum),.SDcols=37:42, by=GeneID]
  focal_qtl_gene_significant<-unique(focal_qtl_gene_significant, by="GeneID")
  focal_qtl_gene_significant<-focal_qtl_gene_significant[,c("LGn","GeneID","gene.name","snp.count","start","stop",
                                "pbs.r","pbs.g","AFD.GR","pbs.r.perc","pbs.g.perc","AFD.GR.perc",
                                "Pos","In.Fibrosis","In.FibrosisORGranuloma","In.Granuloma","In.MaxWormMass","In.ROSresid","In.WormPresent")]
  setkey(focal_qtl_gene_significant,LGn,start)
  fwrite(focal_qtl_gene_significant,"./Analyses_Combined/results/Foen/focal_qtl_gene_significant_bywindow.csv")
}

# ##### Generate a table containing all the genes fall to any QTL
# {
#   PBS_gene[,c("gene_join_start","gene_join_end"):=list(start,stop)]
#   focal_qtl[,c("qtl_join_start","qtl_join_end"):=list(qtl.focal.region.start,qtl.focal.region.end)]
#   focal_qtl_gene<-focal_qtl[PBS_gene,
#                             nomatch = 0L,
#                             on = c("qtl_join_start<=gene_join_start","qtl_join_end>=gene_join_end","LGn"),
#                             allow.cartesian = TRUE]
#   focal_qtl_gene[,avg.sig.gene := FALSE]
#   focal_qtl_gene[(pbs.r.perc>0.99 | pbs.g.perc>0.99) & AFD.GR.perc>0.99,avg.sig.gene := TRUE]
# }   

#################################################################
#  4.  Combine QTL and Popgen results with GWAS and Expression
#################################################################

##### combine with expression
{
  qtl_popgen_expr<-expr[focal_qtl_gene_significant,on="geneID==GeneID",nomatch=0L]
...
##### combine with GWAS
{
  ...
  qtl_popgen_gwas<-foverlaps(focal_qtl_gene_significant,GWAS, by.x = c("LGn","start","stop"), by.y = c("LGn","Start","End"),type="any",nomatch=0L)
...
##### Plot each QTL with PBS sliding window information
{
  # create a list for quantile 99% of pbs and significance level of qtl marker.
  window_statstoplot<-c("PBS.r","PBS.g","AFD.GR")
  window_stats99<-focal_qtl_window[,lapply(.SD,quantile,probs=0.99,na.rm=TRUE),.SDcols = window_statstoplot]
